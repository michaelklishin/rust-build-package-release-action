...
# Tests Debian packages by installing and verifying the binary
...
def main [] {
    ...
    let artifact_path = if $download_from_release {
        ...
        download-artifact $binary_name $version $arch "deb"
    } else {
    ...
    print $"(ansi green)Testing Debian package:(ansi reset) ($artifact_path)"
    print $"(ansi green)Expected version:(ansi reset) ($version)"

    install-deb $artifact_path
    verify-installed-binary $"/usr/bin/($binary_name)" $version
    uninstall-deb $binary_name

    print $"(ansi green)All tests passed(ansi reset)"
    output "result" "success"
}

def install-deb [artifact_path: string] {
    print $"(ansi green)Installing package...(ansi reset)"
    let result = do { sudo dpkg -i $artifact_path } | complete
...
def uninstall-deb [binary_name: string] {
    print $"(ansi green)Uninstalling package...(ansi reset)"
    let result = do { sudo dpkg -r $binary_name } | complete
    if $result.exit_code != 0 {
